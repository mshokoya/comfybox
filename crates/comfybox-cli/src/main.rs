...
const DATASET_QWEN_2509_BASIC_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-qwen-edit-2509-basic-angles.json");
const DATASET_QWEN_2509_FACE_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-qwen-edit-2509-face.json");
const DATASET_QWEN_2509_LIFESTYLE_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-qwen-edit-2509-lifestyle.json");
const DATASET_QWEN_2511_BODY_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-qwen-edit-2511-body-angles.json");
const DATASET_FLUX2_KLEIN_BODY_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-flux2-klein-9b-body-angles.json");
const DATASET_KREA2_LIFESTYLE_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-krea2-lifestyle.json");
const DATASET_POST_PRODUCTION_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-post-production.json");
const DATASET_AUTO_CAPTION_WORKFLOW: &str = include_str!("../../../assets/workflows/dataset-auto-caption.json");
const KREA2_GENERATE_WORKFLOW: &str = include_str!("../../../assets/workflows/krea2-generate.json");
const FACE_SWAP_COMPARISON_WORKFLOW: &str = include_str!("../../../assets/workflows/face-swap-krea2-qwen2511-flux2-klein.json");
const FLUX2_KLEIN_FACE_SWAP_WORKFLOW: &str = include_str!("../../../assets/workflows/flux2-klein-face-swap.json");
const KREA2_FACE_SWAP_WORKFLOW: &str = include_str!("../../../assets/workflows/krea2-face-swap.json");
const MINIMAX_H3_CHARACTER_SWAP_WORKFLOW: &str = include_str!("../../../assets/workflows/minimax-h3-character-swap-bf16-24to60.json");
const QWEN_2511_FACE_SWAP_WORKFLOW: &str = include_str!("../../../assets/workflows/qwen-edit-2511-face-swap.json");

#[derive(Parser, Debug)]
#[command(name="comfybox", version, about="Reliable ComfyUI installer and model/workflow package manager")]
struct Cli {
    #[arg(long, global=true)] catalog: Vec<PathBuf>,
    #[command(subcommand)] command: Option<CommandTop>,
}
...
enum CommandTop {
    ...
    Workflow { #[command(subcommand)] command: WorkflowCommand },
...
struct ModelInstallArgs {
    id: String,
    #[arg(long)] no_deps: bool,
    #[arg(long)] force: bool,
    #[arg(long="optional")] optional_groups: Vec<String>,
}

#[derive(Subcommand, Debug)]
enum WorkflowCommand {
    List,
    Inspect { id: Option<String>, #[arg(long)] file: Option<PathBuf> },
    InstallDeps { id: Option<String>, #[arg(long)] file: Option<PathBuf>, #[arg(long)] force: bool },
    RemoveDeps { id: Option<String>, #[arg(long)] file: Option<PathBuf>, #[arg(long)] yes: bool },
    Install { id: String, #[arg(long)] with_deps: bool, #[arg(long)] force: bool },
}

#[derive(Subcommand, Debug)]
enum ConfigCommand {
    Show,
    SetHfEndpoint { endpoint: String },
    SetComfyPath { path: PathBuf },
    SetHfToken {
...
fn load_catalog(extra: &[PathBuf]) -> Result<Catalog> {
    ...
    if let Ok(dir) = AppConfig::catalog_dir() {
        if dir.is_dir() {
            let mut entries: Vec<_> = fs::read_dir(&dir)?.filter_map(|e| e.ok()).map(|x| x.path()).filter(|p| matches!(p.extension().and_then(|x| x.to_str()), Some("toml"|"json"))).collect();
            entries.sort();
            for p in entries { cat = cat.merge(Catalog::load_file(&p)?)?; }
        }
    }
    for p in extra { cat = cat.merge(Catalog::load_file(p)?)?; }
    Ok(cat)
}
...
async fn run(cmd: CommandTop, cfg: &mut AppConfig, cat: &Catalog) -> Result<()> {
    match cmd {
        ...
        CommandTop::Stop => { let i=configured_instance(cfg)?; let mut s=ManagedState::load()?; ComfyManager::stop(&i,&mut s)?; println!("{}", style("✓ ComfyUI stopped").green()); Ok(()) }
        CommandTop::Status => { let i=configured_instance(cfg)?; let mut s=ManagedState::load()?; let up=ComfyManager::status(&i,&mut s)?; println!("{}", if up { style("running").green() } else { style("stopped").yellow() }); if let Some(log)=s.comfy_log { println!("log: {log}"); } Ok(()) }
        ...
        CommandTop::Catalog { command: CatalogCommand::Validate } => { cat.validate()?; println!("{} {} artifacts, {} packages, {} workflows", style("✓ catalog valid").green(), cat.artifacts.len(),cat.packages.len(),cat.workflows.len()); Ok(()) }
...
async fn doctor(cfg: &AppConfig, cat: &Catalog) -> Result<()> {
    ...
    println!("catalog: {} artifacts / {} packages / {} workflows", cat.artifacts.len(),cat.packages.len(),cat.workflows.len());
    println!("ComfyUI: {}", cfg.comfy_path.as_ref().map(|p| p.display().to_string()).unwrap_or_else(|| "not configured".into()));
    let env_hf = std::env::var("HF_ENDPOINT").ok();
    println!("HF endpoint: {}", cfg.hf_endpoint.as_deref().or(env_hf.as_deref()).unwrap_or("https://huggingface.co"));
    println!("HF token: {}", if auth::resolve_hf_token()?.is_some() { "present" } else { "not set" });
    for bin in ["git", if cfg!(windows) {"python"} else {"python3"}] {
        let ok = Command::new(bin).arg("--version").output().await.map(|o|o.status.success()).unwrap_or(false);
        println!("{bin}: {}", if ok { "ok" } else { "missing" });
    }
    if let Some(d)=storage::recommend_storage()? { println!("largest recommended volume: {} ({:.1} GiB free)",d.mount_point.display(),gib(d.available_bytes)); }
    Ok(())
}

fn locate(cfg: &mut AppConfig, path: Option<PathBuf>) -> Result<()> {
    let selected = if let Some(p)=path { p } else {
        let found = ComfyManager::discover(&[], 5);
        if found.is_empty() { browse_directory(std::env::current_dir()?)? }
        else { Select::new("Select ComfyUI installation", found.iter().map(|x|x.root.display().to_string()).collect()).prompt().map(PathBuf::from)? }
    };
    if !ComfyManager::is_comfy_root(&selected) { bail!("not a ComfyUI root: {}", selected.display()); }
    cfg.comfy_path=Some(fs::canonicalize(selected)?); cfg.save()?;
...
fn uninstall_comfy(cfg:&mut AppConfig,yes:bool)->Result<()> {
    ...
    if !confirmed { println!("cancelled"); return Ok(()); }
    fs::remove_dir_all(&i.root)?; cfg.comfy_path=None; cfg.save()?; println!("{}",style("✓ ComfyUI deleted").green()); Ok(())
}

async fn models(cmd:ModelCommand,cfg:&AppConfig,cat:&Catalog)->Result<()> {
    let i=configured_instance(cfg)?; let inv=Inventory{comfy_root:&i.root,catalog:cat};
    match cmd {
        ModelCommand::List=>{ for p in &cat.packages { let statuses:Vec<_>=p.primary_artifact_ids.iter().filter_map(|id|cat.artifact(id)).map(|a|inv.status(a)).collect(); let mark=if p.primary_artifact_ids.is_empty(){"?"}else if statuses.iter().all(|s|matches!(s,ArtifactStatus::Installed)){"✓"}else if statuses.iter().any(|s|matches!(s,ArtifactStatus::Partial{..})){"◐"}else{"○"}; println!("{} {:<26} {}",style(mark).cyan(),p.id,p.name); } Ok(()) },
        ModelCommand::Install(args)=>install_package(&i.root,cfg,cat,&args.id,!args.no_deps,&args.optional_groups,args.force).await,
        ModelCommand::Remove{id,with_deps,yes}=>remove_package(&i.root,cat,&id,with_deps,yes),
        ModelCommand::Discover{query,limit}=>discover_hf(&query,limit).await,
    }
}

async fn install_package(root:&Path,cfg:&AppConfig,cat:&Catalog,id:&str,with_deps:bool,optional:&[String],force:bool)->Result<()> {
    let p=cat.package(id).with_context(||format!("unknown package {id}"))?;
    if p.primary_artifact_ids.is_empty() { bail!("{} is discovery-only; use `comfybox models discover {}` until a verified manifest is added",p.name,p.discover_query.as_deref().unwrap_or(&p.id)); }
    let mut ids=p.primary_artifact_ids.clone(); if with_deps { ids.extend(p.dependency_artifact_ids.clone()); }
    for gid in optional { let g=p.optional_groups.iter().find(|g|&g.id==gid).with_context(||format!("unknown optional group {gid}"))?; ids.extend(g.artifact_ids.clone()); }
    ids.sort(); ids.dedup();
    ...
    for nid in &node_ids { install_custom_node(root,cat,nid).await?; }
    let mut state=ManagedState::load()?;
    state.packages.insert(p.id.clone(),ManagedInstall{artifact_ids:installed,custom_node_ids:node_ids});
    state.save()?;
    println!("{} {}",style("✓ installed").green(),p.name); Ok(())
}
...
fn remove_package(root:&Path,cat:&Catalog,id:&str,with_deps:bool,yes:bool)->Result<()> {
    ...
    let mut ids:BTreeSet<String>=if let Some(r)=&recorded { r.artifact_ids.clone() } else { p.primary_artifact_ids.iter().cloned().collect() };
    if !with_deps { ids.retain(|x|p.primary_artifact_ids.contains(x)); }
    let confirmed=yes || Confirm::new(&format!("Remove {} managed files?",p.name)).with_default(false).prompt()?;
    if !confirmed { return Ok(()) }
    for aid in ids {
        let shared_in_state = state.artifact_referenced_elsewhere(&aid,Some(id),None);
        let shared_on_disk = catalog_artifact_needed_by_other_installed_package(root, cat, &aid, id);
        ...
        if let Some(a)=cat.artifact(&aid) {
            ...
            if path.exists(){fs::remove_file(&path)?;println!("removed {}",path.display());}
        }
    }
    if with_deps {
        if let Some(rec)=&recorded {
            for nid in &rec.custom_node_ids {
                if state.custom_node_referenced_elsewhere(nid,Some(id),None) { println!("{} shared custom node {nid}",style("keep").yellow()); continue; }
                if let Some(n)=cat.custom_node(nid) {
                    let path=root.join("custom_nodes").join(&n.folder_name);
                    if path.exists(){fs::remove_dir_all(&path)?;println!("removed {}",path.display());}
                }
            }
        }
    }
    state.packages.remove(id); state.save()?; Ok(())
}

fn catalog_artifact_needed_by_other_installed_package(root:&Path,cat:&Catalog,artifact_id:&str,except_id:&str)->bool {
    cat.packages.iter().filter(|p| p.id != except_id).any(|p| {
        let installed = p.primary_artifact_ids.iter().filter_map(|id|cat.artifact(id)).any(|a| root.join(&a.relative_path).exists());
        installed && (p.primary_artifact_ids.iter().chain(&p.dependency_artifact_ids).any(|id|id==artifact_id) || p.optional_groups.iter().flat_map(|g|g.artifact_ids.iter()).any(|id|id==artifact_id))
    })
}
...
async fn install_named_workflow_deps(root:&Path,id:&str,cfg:&AppConfig,cat:&Catalog,force:bool)->Result<()> {
    ...
    let mut state=ManagedState::load()?;
    state.workflows.insert(id.to_string(), ManagedInstall{artifact_ids:w.artifact_ids.iter().cloned().collect(), custom_node_ids:w.custom_node_ids.iter().cloned().collect()});
    state.save()?;
    Ok(())
}

fn resolve_workflow_path(id:Option<&str>,file:Option<PathBuf>,cat:&Catalog)->Result<PathBuf>{if let Some(f)=file{return Ok(f)};let id=id.context("provide workflow id or --file")?;let w=cat.workflow(id).with_context(||format!("unknown workflow {id}"))?;let temp=std::env::temp_dir().join(format!("comfybox-{id}.json"));fs::write(&temp,bundled_workflow(id)?)?;let _=&w.file;Ok(temp)}
fn bundled_workflow(id:&str)->Result<&'static str>{match id{
...
fn save_hf_token(from_env: bool, stdin: bool) -> Result<()> {
    ...
    println!("{} {}", style("✓ HF token saved securely at").green(), path.display());
    println!("It is available to ComfyBox immediately and on future runs.");
    Ok(())
}
fn temp_clean(cfg:&AppConfig)->Result<()> {let i=configured_instance(cfg)?;for e in walkdir::WalkDir::new(&i.root).follow_links(false).into_iter().filter_map(|e| e.ok()){if e.file_type().is_dir()&&e.file_name()==".comfybox-tmp"{println!("removing {}",e.path().display());let _=fs::remove_dir_all(e.path());}}Ok(())}

async fn interactive(cfg: &mut AppConfig, cat: &Catalog) -> Result<()> {
    loop {
        let action = dashboard::run(cfg, cat)?;
        if matches!(action, dashboard::DashboardAction::Quit) {
            return Ok(());
        }

        if let Err(error) = execute_dashboard_action(action, cfg, cat).await {
...
async fn execute_dashboard_action(
    ...
    match action {
        ...
        dashboard::DashboardAction::InstallPythonDeps => {
            ...
            println!("{} {}", style("✓ Python deps installed:").green(), python.display());
            Ok(())
        }
        ...
        dashboard::DashboardAction::ToggleServer => {
            ...
            if ComfyManager::status(&instance, &mut state)? {
                ...
                let pid = ComfyManager::start(
                    &instance,
                    StartOptions { host: "127.0.0.1", port: 8188 },
                    &mut state,
                )
                .await?;
                println!("{} PID {pid}; http://127.0.0.1:8188", style("✓ ComfyUI started").green());
            }
            Ok(())
        }
        dashboard::DashboardAction::InstallPackage(id) => {
            let instance = configured_instance(cfg)?;
            let package = cat.package(&id).with_context(|| format!("unknown package {id}"))?;
            let labels = package
                .optional_groups
                .iter()
            ...
            install_package(&instance.root, cfg, cat, &id, true, &optional, false).await
        }
        dashboard::DashboardAction::InstallWorkflow(id) => {
            workflows(WorkflowCommand::Install { id, with_deps: true, force: false }, cfg, cat).await
        }
        dashboard::DashboardAction::InstallArtifact { id, force } => {
            let instance = configured_instance(cfg)?;
...
fn status_mark(s:&ArtifactStatus)->String{match s{ArtifactStatus::Installed=>style("✓ installed").green().to_string(),ArtifactStatus::Partial{downloaded_bytes,..}=>style(format!("◐ {:.1} GiB partial",gib(*downloaded_bytes))).yellow().to_string(),ArtifactStatus::SizeMismatch{..}=>style("! mismatch").red().to_string(),ArtifactStatus::Missing=>style("○ missing").dim().to_string()}}
fn browse_directory(start:PathBuf)->Result<PathBuf>{let mut cur=start;loop{let mut dirs:Vec<PathBuf>=fs::read_dir(&cur)?.filter_map(|e| e.ok()).map(|e|e.path()).filter(|p|p.is_dir()).collect();dirs.sort();let mut labels=vec!["[select this directory]".to_string()];if cur.parent().is_some(){labels.push("[..]".into())}labels.extend(dirs.iter().map(|p|format!("[{}]",p.file_name().unwrap_or_default().to_string_lossy())));let choice=Select::new(&format!("Directory: {}",cur.display()),labels.clone()).prompt()?;if choice=="[select this directory]"{return Ok(cur)}
if choice=="[..]"{cur=cur.parent().unwrap().to_path_buf();continue}let name=choice.trim_start_matches('[').trim_end_matches(']');cur=cur.join(name)}}
fn gib(b:u64)->f64{b as f64/1024.0/1024.0/1024.0}
fn uuid_like()->String{use std::time::{SystemTime,UNIX_EPOCH};format!("{}-{}",std::process::id(),SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos())}
